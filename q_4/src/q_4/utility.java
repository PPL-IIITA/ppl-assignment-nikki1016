...
public class utility {
     ...
     * @param bw1 
     */
    void leastKHappiestCouple(couples couple[],int noOfCouple,int k,boys arr_boys[],girls arr_girls[],BufferedWriter bw1){
        try{
            int i,j;
            couples temp ;
        //sort couples as per ther happiness in ascending order
            for(i=0; i < noOfCouple -1; i++){
                for(j= i+1; j< noOfCouple ; j++){
            ...
            bw1.newLine();
            // breakup all k least happiest couples and allocate new boyfriends to those girls

            for(i = noOfCouple-1; i > noOfCouple - k-1; i--){
                if(couple[i] == null)
                    continue;
                System.out.println("Couple: "+couple[i].boy.bname + " and " + couple[i].girl.gname+" breaks up.");
                String br =new String("Couple: "+couple[i].boy.bname + " and " + couple[i].girl.gname +" breaks up.");
                ...
                // allocate new boyfriend other than her ex
                if(allocate_notex(arr_boys,arr_girls[in]))
                {
                    couple[i] = new couples(arr_girls[in].bf,girl);
    ...
    void leastKHappiestCouple(couples couple[],int noOfCouple,int k,boys arr_boys[],girls arr_girls[],BufferedWriter bw1,Timestamp tt){
        try{
            int i,j;
            couples temp ;
            for(i=0; i < noOfCouple -1; i++){
    ...
    boolean allocate_notex( boys arr_boys[], girls girl) {
            ...
            for(j=0; j< arr_boys.length; j++){
                if(available_notex(girl,arr_boys[j])){
                    girl.bf = arr_boys[j];
                    girl.status = true;
                    arr_boys[j].gf = girl;
